...
class HTML :        
    ...
    def find_value_from_HTML_atFirst(self, temp , index): 
    ...
    def find_value_overOne(self, temp , flag, index):
    ...
    def parsing_html(self):
        ...
        for product_no in range(start_loop_num, end_loop_num):
            response = requests.get(
                f"{test_url_lists[0]}/product/detail.html?product_no={product_no}", headers=header)
            ...
            if response.status_code == 200:
                ...
                if frame != None:
                    ...
                    for i in range(len(parsing_list)):
                        ...
                        wanted_value_list.append(find_value_overOne(parsing_list[i], flag_index_list[i][0], flag_index_list[i][1]))
